...
export const requestProductSchema = z.object({
  ...
  referenceImages: z
    ...
    .refine(
      ...
      {
        message: "Each image maximum 1MB",
      }
    ),
}).refine(
  (data) => {
    ...
    switch (contactPlatform) {
      ...
      case "whatsapp": {
        ...
        if (contact.startsWith("+0")) {
          throw new z.ZodError([
            {
              code: "custom",
              message: "Invalid format. Use + for international or 0 for local, not both",
              path: ["contact"],
            },
          ]);
        }
        ...
        if (invalidChars.length > 0) {
          throw new z.ZodError([
            {
              code: "custom",
              message: "Only numbers, spaces, dashes, and parentheses allowed",
              path: ["contact"],
            },
          ]);
        }
      ...
      case "instagram": {
        const instaRegex = /^(https?:\/\/)?(www\.)?instagram\.com\/[a-zA-Z0-9._]+\/?$|^@?[a-zA-Z0-9._]{3,30}$/;
        if (!instaRegex.test(contact)) {
          throw new z.ZodError([
            {
              code: "custom",
              message: "Please enter a valid Instagram username (e.g., @username) or profile URL",
              path: ["contact"],
            },
          ]);
        }
      ...
      case "facebook": {
        const fbRegex = /^(https?:\/\/)?(www\.)?facebook\.com\/[a-zA-Z0-9.]+\/?$|^[a-zA-Z0-9.]{3,}$/;
        if (!fbRegex.test(contact)) {
          throw new z.ZodError([
            {
              code: "custom",
              message: "Please enter a valid Facebook username or profile URL",
              path: ["contact"],
            },
          ]);
        }
